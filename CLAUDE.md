...
| [docs/COMPLIANCE_REFERENCE.md](docs/COMPLIANCE_REFERENCE.md) | **Read before changing DVIR, RODS, PODS, DQ, BOL, the fleet registry, or the Sheets export.** Maps each compliance-bearing code surface to the federal record it produces and the invariants that must not regress, above an applicability gate saying which rules reach this company at all. Part 2 is the open gap ledger from the 2026-09 audit. **It is not a statement of law**: claims are marked `[code]`, `[owner]` or `[reg]`, and a `[reg]` mark is a research lead. No CFR text is vendored ([ADR 0051](docs/decisions/0051-the-compliance-reference-maps-code-to-obligations-and-vendors-no-cfr.md)). |
| [docs/bugs/](docs/bugs/README.md) | The bug ledger: one row per defect ever found, on `main` or `staging`, with how it was detected (`vet` / `field-reported` / `owner-reported` / `other`) and how long it was live in prod, beside the app's size (lines of code) and dated feature list, for bug rates over time. `scripts/bug_ledger_summary.py` prints the tables. |
...
5. **Found a bug you did not fix?** → add it to Known defects in `docs/RUNBOOKS.md`. **Fixed one that is listed?** → delete the entry. **Fixed any defect?** → add or close its row in [docs/bugs/BUG_LEDGER.csv](docs/bugs/BUG_LEDGER.csv), including how it was detected and who reported it. A fix with no ledger row is a hole in the incidence trend. **Shipped a new feature to `main`?** → add its row to [docs/bugs/FEATURES.csv](docs/bugs/FEATURES.csv).
