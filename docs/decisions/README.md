...
| [0030](0030-bol-inherits-actual-inventory.md) | An empty BOL inherits the job's Actual Inventory | Two separate inventory stores meant crew logged inventory the BOL never read, then signed an empty BOL. An unsigned BOL with no items seeds from `job_inventory_items`. Amends ADR 0015's strict separation. |
