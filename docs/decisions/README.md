...
| [0044](0044-bill-lines-are-verified-one-at-a-time.md) | Bill lines are verified one at a time, and a tick expires when the line changes | One checkbox covering the whole bill is the cheapest thing to satisfy without reading it, and a "Truck #1 (per hour)" line frozen at 1 hour on a $90/hr charge went out under exactly that. Per-line ticks that expire when the line changes. Do not collapse it back to one confirmation. |
| [0045](0045-corrections-are-scoped-by-period-or-by-date-never-by-job.md) | A correction is scoped by period or by date, and the discriminator is the period, not the job | Both read paths keyed date-scoping on `job_uuid` because the two happened to coincide. Giving off-job hours a correction surface broke it: the row was applied by nobody and mailed by nobody, so pay would have changed silently. Amends 0032. |
| [0046](0046-every-change-goes-through-a-direction-intake.md) | Every change goes through a direction and context intake, no size exception | Governs how the assistant works here, not the app. Ask direction as a multi-select, capture the crew or admin scenario in the owner's words, record which user classes it touches. Unconditional on purpose: deciding what is "big enough" to ask about is itself the leak. Produces `PRD.md` and `USER_PROFILES.md`. |
| [0047](0047-the-day-plan-is-per-day-data-and-says-when-it-is-missing.md) | The day plan is per-day data, editable in place, and says when driving is missing | Per-day, not per-trip. A missing driving leg is stated rather than left blank, so an absent plan is distinguishable from an unanswered one. |
| [0048](0048-a-photo-is-durable-when-it-is-taken-not-when-it-is-saved.md) | A photo is durable when it is taken, not when Save is pressed | The file handle is read at capture, not at submit. A dead handle at submit threw before the row was enqueued, so the photo was lost and the tray cleared. Do not move the read back to submit. |
| [0049](0049-a-sheet-row-is-written-atomically-and-a-lying-marker-is-cleared-by-hand.md) | A Sheet row is written atomically, and a lying marker is cleared by hand | A marker written before the row means the record looks exported and is absent. Write the row first. |
| [0050](0050-colorblind-availability-palettes-live-on-the-account-and-recolor-only-your-own-view.md) | Colorblind availability palettes live on the account, and recolor only your own view | Per-person and cross-device, unlike theme settings which are per-device. Admin views stay conventional. |
| [0051](0051-the-compliance-reference-maps-code-to-obligations-and-vendors-no-cfr.md) | The compliance reference maps code to obligations | Organized by code surface, not CFR part. No regulation text is vendored; a frozen copy goes stale while looking authoritative. |
| [0052](0052-a-rods-row-is-always-signed-and-the-server-is-what-says-so.md) | A RODS row is always signed, and the server is what says so | The signed-only rule used to hold because the client happened to submit after signing. Three layers refuse an unsigned day now. Do not re-open the endpoint to build draft autosave: drafts get their own table. |
| [0053](0053-a-rental-is-a-placeholder-and-the-job-records-which-truck-it-was.md) | A rental is a placeholder, and the job records which truck it was | One generic "rental" unit put the word "rental" in the DVIR, RODS and BOL as the vehicle, and merged every hired truck into one inspection history. The job now records the plate and GVWR. Do not drop vehicle_identifier from the prior-report query. |
| [0054](0054-every-release-is-copied-to-the-owners-vault.md) | Every release is copied to the owner's vault | A file snapshot of main and staging at each merge to main, newest five kept. Not every push, no git history, no .env files. |
| [0055](0055-a-rental-truck-is-entered-once-and-offered-in-the-truck-list.md) | A rental truck is entered once, and offered in the truck list | Amends 0053. Each rented truck is its own record, entered wherever it first matters (usually the pre-trip DVIR), listed as "Rental*<job name>", linked to every job it serves. Do not let a later save blank a detail, unlink a truck, or change an inspected plate. |
