...
- The setting is on My Profile, and the Availability Submit tab carries a link to
  it. The owner chose "always"; as built it shows whenever the Submit picker does,
  which is only while a window is due or unlocked (sanity 2026-09-14 F1, declined,
  recorded in RUNBOOKS Known defects). Back from the setting returns to
  Availability (F3, fixed the same day). The link reads "Hard to tell the colors apart? Colorblind-friendly
  colors" and opens `/profile#colors`. Added 2026-09-14 after the owner reviewed the
