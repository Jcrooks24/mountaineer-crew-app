...
Origin Signing (before loading): enter the origin (pickup) and destination (delivery) addresses, capture the actual pickup date and the vehicle (unit) for this trip, the client's printed name, both signatures, and check the electronic-signature consent box. Tap "Sign at origin & give copy." The addresses are required: they print on the BOL because a DOT officer at a border crossing needs to see the pickup and delivery addresses on the signed document.
