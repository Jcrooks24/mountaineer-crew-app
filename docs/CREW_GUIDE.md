...
**Employee directory.** Tools > Employee directory searches current employees by name, email or phone. Tap a number to call it or an address to email it. Phone matching ignores punctuation, so "(555) 0134" and "5550134" both find the same person. It uses the roster your phone has already cached, so it works without signal once you have been online once. If somebody is missing or a number is wrong, the office fixes it on the roster.
