...
- **DVIR unit list:** add, edit, remove vehicles from the DVIR dropdown. Tick **This is a rental placeholder** on the generic rental entry: that entry becomes "+ New rental truck" in the unit lists, and each truck the crew or you enter behind it becomes its own record, listed as "Rental*<job name>" until it is returned or unused for 10 days. The RentalTrucks tab lists every rental with every job it served. On a job's setup you can pick a listed rental, enter one at booking, mark one returned, or remove a wrong pick (refused once the truck has an inspection on that job). (ADR 0055)
...
- **Job Checklist:** the checklist shown on a job. Each item is either manual (the crew tick it) or auto (it ticks itself when the app sees the thing happen - a DVIR filed, the report saved, the BOL signed, PODS/RODS filed, etc.). Limit an item to long-distance jobs, to jobs with a truck ("Only if a truck is on the job"), and/or to specific job types; leave job types empty for every job. The truck limit reads the vehicle units on the job's setup header, so a pre-trip DVIR or "Trucks swept out" stops showing as an outstanding task on a labor-only job that never had a truck. **It is off on every existing item and is not back-filled** - tick it yourself on the items that need it (both DVIRs, Trucks swept out, DOT markings, Weighed at a certified scale). A job with no setup header saved yet counts as having a truck, so nothing disappears before the office has filled the header in. The crew see the applicable items on the hub once a job is set up
